...
const RUNTIME = process.env.RUNTIME_NAME || 'bun';
...
const WRITE_DATA = Buffer.alloc(
  2 * 1024 * 1024,
  66
);

const TEST_PASSWORD =
  'diploma-benchmark-test-password';

const TEST_HASH = bcrypt.hashSync(
  TEST_PASSWORD,
  8
);
...
if (!fs.existsSync(DATA_DIR)) {
  throw new Error(
    `Podatkovna mapa ne obstaja: ${DATA_DIR}`
  );
}
...
async function shutdown(signal) {
  ...
  console.log(
    `[SHUTDOWN] Prejet signal ${signal}`
  );

  const forceShutdownTimer =
    setTimeout(() => {
      console.error(
        '[SHUTDOWN] Prisilna ustavitev procesa'
      );
